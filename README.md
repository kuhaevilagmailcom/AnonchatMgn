...
- запрет телефонов и email с явным разрешением username и ссылок;
...
`/start` — начать · `/connect` — поиск · `/next` — следующий · `/stop` — выйти · `/report` — жалоба · `/send @username` или `/user @username` — отправить username/ссылку собеседнику · `/profile` — профиль · `/settings` — настройки · `/support` — поддержать проект звёздами · `/help` — помощь · `/forget` — удалить данные.

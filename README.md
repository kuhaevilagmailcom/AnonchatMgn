...
- публичный ник и Telegram username собеседника, если username установлен; Telegram ID скрыт;
...
`/start` — меню · `/connect` — поиск · `/next` — следующий · `/stop` — выйти · `/report` — жалоба · `/send @username` — поделиться контактом · `/profile` — профиль и реферальная ссылка · `/settings` — настройки · `/ref` — реферальная ссылка · `/support` — поддержать проект · `/help` — помощь · `/forget` — удалить профиль.

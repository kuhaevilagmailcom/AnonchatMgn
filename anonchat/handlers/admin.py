...
async def cmd_reports(message: Message, ctx: Ctx, db: Database) -> None:
    ...
    for r in rows:
        await ctx.reply(
            ...
            f"На: <code>{r['target_id']}</code> ({texts.esc(r['target_name'] or '')})\n"
...
async def cmd_ban(message: Message, ctx: Ctx, db: Database, mm: Matchmaker, cfg: Config) -> None:
    ...
    await break_pair(ctx.bot, cfg, mm, uid, "🛡 Собеседник отключился :(")
    await send_to(ctx.bot, uid, texts.BANNED.format(city=cfg.city, reason=reason, contact="/feedback"))
...
async def cmd_mute(message: Message, ctx: Ctx, db: Database, mm: Matchmaker, cfg: Config) -> None:
    ...
    await break_pair(ctx.bot, cfg, mm, int(parts[0]), "🛡 Модерация закрыла диалог.")
...
async def cmd_find(message: Message, ctx: Ctx, db: Database) -> None:
    ...
    for r in rows:
        ...
        lines.append(
            f"<code>{r['user_id']}</code> · {texts.esc(r['first_name'])} "
            f"({texts.esc(r['username'] or '-')})\n"
            f"   🥇 {info.level} ур. · {info.xp} XP · 💬 {r['dialogs']} · 🚩 {r['reports_received']}"
...
async def cmd_broadcast(message: Message, ctx: Ctx, db: Database, cfg: Config) -> None:
    ...
    if not body:
        await ctx.reply("Формат: <code>/bc 🎄 В МГН выпал первый снег — болтайте тёпло!</code>")
        return
    ids = await db.active_ids(days=7)
    await ctx.reply(f"📣 Рассылаю {len(ids)} адресатам…")
    sent = 0
    for uid in ids:
        if await send_to(ctx.bot, uid, f"📣 {body}", K.menu_keyboard(cfg.emoji_pack_url)):
            sent += 1
...
async def cb_admin(event: CallbackQuery, ctx: Ctx, db: Database, mm: Matchmaker, cfg: Config) -> None:
    ...
    if action == "who":
        ...
        await ctx.reply(
            f"👤 <code>{target}</code> · {texts.esc(row['first_name'])} "
            f"({texts.esc(row['username'] or '-')})\n"
            f"🥇 {info.level} ур. · {info.xp} XP · «{texts.esc(info.title)}»\n"
            f"💬 Диалогов: {row['dialogs']} · ✉️ Сообщений: {row['messages']}\n"
    ...
    if action == "mute":
        ...
        await send_to(ctx.bot, target, texts.MUTED.format(mins=60))
        await break_pair(ctx.bot, cfg, mm, target, "🛡 Модерация закрыла диалог из-за жалобы.")
        await db.resolve_report(int(raw_id), ctx.user_id)
    ...
    if action == "ban":
        ...
        await break_pair(ctx.bot, cfg, mm, target, "🛡 Собеседник отключился :(")
        await send_to(
            ctx.bot,
            target,
            texts.BANNED.format(city=cfg.city, reason=f"жалоба #{raw_id}", contact="/feedback"),
        )
        await db.resolve_report(int(raw_id), ctx.user_id)

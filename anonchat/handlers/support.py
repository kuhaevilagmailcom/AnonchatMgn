...
async def show_premium(ctx: Ctx, cfg: Config) -> None:
    ...
    active = nicklib.is_premium(premium_until)
    status = (
        f"\n\nАНОН+ активен до <b>{time.strftime('%d.%m.%Y', time.localtime(premium_until))}</b>."
        if active else ""
    )
    body = (
        "<b>АНОН+</b>\n\n"
...
async def cb_buy_premium(event: CallbackQuery, ctx: Ctx, cfg: Config) -> None:
    ...
    await target.answer_invoice(
        title=f"АНОН+ · {cfg.premium_days} дней",
        description="Значок ✦, статус и дополнительная статистика.",
        payload=payload,
        currency="XTR",
        prices=[LabeledPrice(label="АНОН+", amount=cfg.premium_price_stars)],
        provider_token="",
    )
    await ctx.ack()
...
def _valid_payload(query: PreCheckoutQuery, cfg: Config) -> bool:
    ...
    return (
        ...
        and (
            (parts[0] == "support" and MIN_STARS <= value <= MAX_STARS and query.total_amount == value)
            or (
                parts[0] == "premium"
                and value == cfg.premium_days
                and query.total_amount == cfg.premium_price_stars
            )
        )
    )
...
async def successful_payment(message: Message, ctx: Ctx, cfg: Config, db: Database) -> None:
    ...
    if kind == "premium":
        ...
        await ctx.reply(f"АНОН+ активен до <b>{until}</b> ✦", K.menu_keyboard())
    else:
        await ctx.reply(

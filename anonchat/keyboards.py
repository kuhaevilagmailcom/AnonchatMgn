"""Инлайн-клавиатуры. Эмодзи — по одному на кнопку: это маркер, а не украшение."""
...
def menu_keyboard(status: str = "free", queue_size: int = 0) -> InlineKeyboardMarkup:
    """Главное меню: сверху доминирует поиск/статус, остальное сеткой 2×2."""
    b = InlineKeyboardBuilder()
    if status == "paired":
        head = "💬 Диалог идёт"
    elif status == "queued":
        head = f"⏳ В очереди · ждёт пары: {queue_size}"
    else:
...
def settings_keyboard(
    ...
    b.button(
        text="🧭 Ищу: мой район" if same_district else "🧭 Ищу: весь город",
        callback_data="cfg:same:toggle",
    )
    b.button(text="✍️ Описание" if has_about else "✍️ Заполнить «о себе»", callback_data="cfg:about:ask")
    b.button(text="👤 Пол в профиле", callback_data="cfg:gender:ask")
    b.button(text="♻️ Сбросить настройки", callback_data="cfg:reset")
...
REASON_TITLES = {code: title.split(" ", 1)[1] for title, code in REPORT_REASONS}
...
def confirm_stop_keyboard() -> InlineKeyboardMarkup:
    ...
    b.button(text="↩️ Продолжить", callback_data=CB_STOP_NO)
    b.adjust(2)
    return b.as_markup()
...
def confirm_forget_keyboard() -> InlineKeyboardMarkup:
    ...
    b.button(text="✖️ Отмена", callback_data="cfg:forget:no")
    b.adjust(2)
    return b.as_markup()
